...
function EditRoom({ room }) {
    ...
    const onEdit = () => {
        ...
        const newPreferredTemperature = document.getElementById('newPreferredTemperature')?.value
        const newPreferredHumidity = document.getElementById('newPreferredHumidity')?.value
            ...
            <div className="bg-white bg-opacity-15 h-full text-xs md:text-sm lg:text-base flex flex-col justify-center items-center rounded-md">
                <form className="flex flex-col w-2/3 my-5">
                    <div className="flex items-center">
                        ...
                        <input className="w-full h-8 my-1 p-1 rounded-md focus:outline-none placeholder-gray-300 focus:opacity-75 hover:opacity-75" type="text" id="newRoomName" name="roomName" placeholder="Room name" defaultValue={name} required />
                    </div>
                    <div className="flex items-center">
                        <label className="mr-2 font-bold text-white">Device Id:</label>
                        <input className="w-full h-8 my-1 p-1 rounded-md focus:outline-none placeholder-gray-300 focus:opacity-75 hover:opacity-75" type="number" id="newDeviceId" name="deviceId" placeholder="Device Id" defaultValue={deviceId} required />
                    </div>
                    <div className="flex items-center">
                        <label className="mr-2 font-bold text-white">Preferred Temperature:</label>
                        <input className="w-full h-8 my-1 p-1 rounded-md focus:outline-none placeholder-gray-300 focus:opacity-75 hover:opacity-75" type="number" id="newPreferredTemperature" name="temperature" placeholder="Preferred temperature (°C)" min="5" max="40" defaultValue={preferedTemperature} required />
                    </div>
                    <div className="flex items-center">
                        <label className="mr-2 font-bold text-white">Preferred Humidity:</label>
                        <input className="w-full h-8 my-1 p-1 rounded-md focus:outline-none placeholder-gray-300 focus:opacity-75 hover:opacity-75" type="number" id="newPreferredHumidity" name="humidity" placeholder="Preferred humidity (%)" min="0" max="100" defaultValue={preferedHumidity} required />
                    </div>
