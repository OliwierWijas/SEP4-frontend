...
function House({ setTemperature, setHumidity, setLightLevel, setRoom, setCreateRoomOpen }) {
    const [width, setWidth] = useState('');
    const [rooms, setRooms] = useState(roomData); // Initialize state with roomData
    ...
    const handleDeleteRoom = (roomId) => {
        setRooms(prevRooms => prevRooms.filter(room => room.id !== roomId)); // Filter out the room with the given id
    };
            ...
            <div className="house w-4/5 max-h-[478px] md:max-h-full flex flex-wrap justify-between mx-auto mt-1 pb-3 overflow-y-auto md:overflow-y-none scrollbar">
                {rooms && rooms[0] && rooms?.map((room) => (
                    <div key={room.id} className='roomDiv flex w-full md:w-1/2 lg:w-1/3 px-1 my-1 justify-center'>
                        <Room room={room} setTemperature={setTemperature} setHumidity={setHumidity} setLightLevel={setLightLevel} setRoom={setRoom} onDelete={handleDeleteRoom} />
                    </div>
                ))}
