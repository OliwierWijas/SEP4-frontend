...
function HouseMembersComponent({ member, onDeleteMember }) {
    ...
    return (
        <div className="bg-white bg-opacity-15 h-fit w-95 mx-2 my-2 rounded-md flex md:items-center text-wrap" data-testid="member-item">
            <div className="flex-grow my-2 mx-2 text-justify md:text-left">{member.username}</div>
            <div>
                <IoTrashOutline onClick={handleDelete} className="mr-2" />
            </div>
        </div>
    );
}
